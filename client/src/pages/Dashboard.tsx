...
export default function Dashboard() {
  ...
  // Dynamically populate signal filter options from actual signals in the database
  const signalFilterOptions = useMemo(() => {
    ...
    sortedSignalDistribution.forEach((item) => {
      if (item.signal && item.signal.trim()) {
        opts.add(item.signal);
      }
    });
    // Add "No Signal" option for filtering companies without signals
    opts.add("No Signal");
  ...
  const filteredCompanies = useMemo(() => {
    ...
    // Signal filter
    if (signalFilter !== "all") {
      if (signalFilter === "No Signal") {
        // Filter for companies without a signal
        filtered = filtered.filter(company => !company.latestSignal);
      } else {
